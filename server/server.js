...
async function handleJoinRoom(args, callback) {
  ...
  const roomUpdate = {
    ...room,
    players: [...room.players, { id: this.id, username: this.data?.username }],
...
function handleMove(args) {
  ...
  if (room.toPlay === "white") {
    ...
    if (room.whiteTime <= 0) {
      return io.to(room.players[0].id).to(room.players[1].id).emit("timeOut", {
        winner: "Black",
      });
    }
